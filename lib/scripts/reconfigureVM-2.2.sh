...
if [ "$LETSENCRYPT" = "true" ]; then
  ...
  certbot certonly --non-interactive -a webroot --webroot-path /var/www/html/letsencrypt --domain $NEW_DOMAIN --email tech@arawa.fr --agree-tos
  #Reconfigure nginx
...
#FILES="/root/bbb-exporter/secrets.env /etc/kurento/modules/kurento/WebRtcEndpoint.conf.ini /opt/freeswitch/conf/vars.xml /usr/share/red5/webapps/sip/WEB-INF/bigbluebutton-sip.properties /etc/bigbluebutton/nginx/sip.nginx /usr/local/bigbluebutton/bbb-webrtc-sfu/config/default.yml /etc/hosts /opt/freeswitch/conf/sip_profiles/external.xml /etc/nginx/conf.d/bigbluebutton_sip_addr_map.conf /etc/nginx/sites-available/bigbluebutton /opt/freeswitch/etc/freeswitch/sip_profiles/external-ipv6.xml"
FILES="/etc/nginx/sites-available/bigbluebutton /root/bbb-exporter/secrets.env"
for FILE in $FILES; do
  echo "- $FILE"
  if [ -f "$FILE" ]; then
    #sed -i -r -e "s/${OLD_EXTERNAL_IPV4}/${NEW_EXTERNAL_IPV4}/g" -e "s/${OLD_EXTERNAL_IPV6}/${NEW_EXTERNAL_IPV6}/g" -e "s/${OLD_INTERNAL_IPV4}/${NEW_INTERNAL_IPV4}/g" -e "s/${OLD_DOMAIN}/${NEW_DOMAIN}/g" "$FILE"
...
# Make it permanent on reboots
echo "@reboot ip addr add $NEW_EXTERNAL_IPV4/32 dev lo && sleep 60 && bbb-conf --restart" | crontab -
